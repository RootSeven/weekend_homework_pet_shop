...
#
  ...
  def test_sell_pet_to_customer__pet_found
    customer = @customers[0]                        # returns hash Alice, cash: 1000
    pet = find_pet_by_name(@pet_shop,"Arthur")      # returns hash Arthur, price: 900
    sell_pet_to_customer(@pet_shop, pet, customer)

    assert_equal(1, customer_pet_count(customer))   # add_pet_to_customer(customer, pet)
    assert_equal(1, pets_sold(@pet_shop))           # pet_shop pets_sold increases by 1
    assert_equal(100, customer_cash(customer))      # customer cash decreases by pet price
    assert_equal(1900, total_cash(@pet_shop))       # pet_shop cash increases by pet price
  end # PASS

  def test_sell_pet_to_customer__pet_not_found
    customer = @customers[0]                        # returns hash Alice, cash: 1000
    pet = find_pet_by_name(@pet_shop,"Dave")        # returns nil
    sell_pet_to_customer(@pet_shop, pet, customer)

    assert_equal(0, customer_pet_count(customer))
    assert_equal(0, pets_sold(@pet_shop))
    assert_equal(1000, customer_cash(customer))
    assert_equal(1000, total_cash(@pet_shop))       # No effect
  end

  def test_sell_pet_to_customer__insufficient_funds
    customer = @customers[1]                        # returns hash Bob, cash: 50
    pet = find_pet_by_name(@pet_shop,"Arthur")      # returns hash Arthur, price: 900
    sell_pet_to_customer(@pet_shop, pet, customer)
